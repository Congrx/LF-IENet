...
class Resize(object):
    ...
    def _resize_dis(self, results):
        """Resize semantic segmentation map with ``results['scale']``."""
        for key in results.get('dis_fields', []):
            if self.keep_ratio:
                gt_dis = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
            else:
                gt_dis = mmcv.imresize(
                    results[key], results['scale'], interpolation='nearest')
            scale = results['scale'][1] / results[key].shape[0]
            gt_dis = gt_dis * scale
            results[key] = gt_dis
...
class RandomRotate(object):
    ...
    def __call__(self, results):
        ...
        if rotate:
            ...
            # rotate sequence
            results['sequence_imgs'] = [mmcv.imrotate(
                img,
                angle=degree,
                border_value=self.pal_val,
                center=self.center,
                auto_bound=self.auto_bound) for img in results['sequence_imgs']]

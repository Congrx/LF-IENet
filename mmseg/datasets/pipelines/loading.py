...
class LoadImageFromFile(object):
    ...
    def __call__(self, results):
        ...
        if results.get('sequence_prefix') is not None:
            sequence_imgs = []
            sequence = results['sequence_info']['sais']       
            for f in sequence:     
                frame_filename = osp.join(results['sequence_prefix'],f)   
        ...
        if results.get('dis_prefix') is not None:
            dis_filename = osp.join(results['dis_prefix'],results['dis_info']['dis_map'])
            disparity = np.load(dis_filename) 
            results['disparity'] = disparity         
            results['dis_fields'].append('disparity')
        ...
        results['sequence_index'] = np.array(results['sequence_index']['sais_index'])  
...
class LoadAnnotations(object):
    ...
    def __call__(self, results):
        ...
        gt_semantic_seg = np.load(filename) - 1

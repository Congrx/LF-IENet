    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            if key == 'sequence_imgs':
                ...
                dis = results[key]
                if len(dis.shape) < 3:
                    dis = np.expand_dims(dis, -1)
                results[key] = to_tensor(dis.transpose(2, 0, 1))
            else:
                sequence_index = results[key]
                results[key] = to_tensor(sequence_index)
        return results
...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'disparity' in results:
            # convert to float32
            results['disparity'] = DC(
                to_tensor(results['disparity'][None,
                                                     ...].astype(np.float32)),
                stack=True)
        if 'sequence_index' in results:
            # convert to float32
            results['sequence_index'] = DC(
                to_tensor(results['sequence_index'][None,
                                                     ...].astype(np.float32)),
                stack=True)
...
class Collect(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_filename', 'ori_shape',
                            ...
                            'flip_direction', 'img_norm_cfg', 'sequence_filename',)):
        self.keys = keys
